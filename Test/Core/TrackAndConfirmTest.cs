...
namespace Meyer.UspsCore.Test.Core
{
	public class TrackAndConfirmTest : Test
	{
		...
		[Fact(DisplayName = "Track Fields Bad Packages ID")]
		...
		[Fact(DisplayName = "Track Fields Single ID")]
		...
		[Fact(DisplayName = "Track Fields double ID")]
		public async Task Test8Async()
		{
			var tracking = new TrackAndConfirm(ApiOptions);

			var results = await tracking.TrackByFieldsAsync(revision: 1,
				clientIp: "192.168.0.1",
				sourceId: "Janglin Corporation",
				destinationZipCode: null,
				mailingDate: null,
